...
~/.codex/AGENTS.md between the canopus:bootstrap:start and
...
5. At the target project root, follow its `AGENTS.md` as the routing map.
6. Inspect Git identity, remote, branch, and worktree before editing.

## Task discipline

- A formal task is admitted from its task Issue:
  `python3 <canopus-root>/tools/canopus_task.py admit --task <id> --envelope <file.json>`
  (idempotent: an existing task resumes instead of restarting).
- Record outcomes with `canopus_task.py event --task <id> --json '<event>'` and
  close each work cycle with `canopus_task.py checkpoint --task <id>`.
- This engine has no Stop hook. Before ending a substantial turn, render
  `python3 <canopus-root>/tools/canopus_task.py footer --task <id>` and make it the
  final block of the response. If it cannot be rendered, end with
  `FOOTER_UNAVAILABLE: <reason>`.
- Resume from the task Issue and `canopus_task.py handoff`, never chat history.
- Stop at `COMPLETE`; on `REALIGN` do alignment only; on `CIRCUIT_BREAK` hand
  back to the owner.

## Boundaries

- One writer per checkout. Simultaneous writers use separate branches and
  worktrees; otherwise stay read-only as reviewer.
